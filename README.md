...
As mentioned, before the attack can be used to infer the membership status of given data points, the attack model needs to be fitted. This can be done via the `fit` method of the `MembershipInferenceBlackBoxAttack`. The `fit` method needs two separate data sets. One that was used to train the target model (i.e. parts of _(X,y)<sub>target</sub>_), and one that was not used to train the target model (i.e. parts of _(X,y)<sub>non-target</sub>_). Both data sets, called _(X,y)<sub>attack</sub>, are then used to train the attack model. To optimize the attack model, consider providing a suitable training set _(X,y)<sub>attack</sub>_.
...
The implementation provided (i.e. [MembershipInferenceLabelOnlyDecisionBoundaryAttack](https://github.com/privML/privacy-evaluator/blob/main/privacy_evaluator/attacks/membership_inference/label_only_decision_boundary.py)) uses ARTs [LabelOnlyDecisionBoundary](https://github.com/Trusted-AI/adversarial-robustness-toolbox/blob/1.7.1/art/attacks/inference/membership_inference/label_only_boundary_distance.py). As mentioned, before the attack can be used to infer the membership status of given data points, the attack model needs to be trained. This can be done via the `fit` method of the `MembershipInferenceLabelOnlyDecisionBoundaryAttack`. The `fit` method needs two separate data sets. One that was used to train the target model (i.e. parts of _(X,y)<sub>target</sub>_), and one that was not used to train the target model (i.e. parts of _(X,y)<sub>non-target</sub>_). Both data sets, called _(X,y)<sub>attack</sub>, are then used to train the attack model. To optimize the attack model, consider providing a suitable training set _(X,y)<sub>attack</sub>_. Additional parameters to optimize the attack model can be found in the documentation of the `fit` method of the `MembershipInferenceLabelOnlyDecisionBoundaryAttack`.
